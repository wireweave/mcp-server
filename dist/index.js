...
async function callApi(endpoint, args) {
  ...
  if (!response.ok) {
    ...
    const userMessage = response.status === 401 ? "Invalid API key. Get one at https://dashboard.wireweave.org" : response.status === 402 ? `Insufficient credits. ${error.message || "Please add more credits."}` : response.status === 403 ? "Access denied. Upgrade your plan for this feature." : response.status === 429 ? "Rate limit exceeded. Please wait and try again." : response.status >= 500 ? "Service temporarily unavailable" : error.error || error.message || "Request failed";
  ...
  if (creditInfo.balance || creditInfo.totalAvailable) {
    return {
      ...result,
      _credits: {
        balance: creditInfo.balance ? parseInt(creditInfo.balance, 10) : void 0,
        monthlyRemaining: creditInfo.monthlyRemaining ? parseInt(creditInfo.monthlyRemaining, 10) : void 0,
        totalAvailable: creditInfo.totalAvailable ? parseInt(creditInfo.totalAvailable, 10) : void 0
      }
    };
  }
  return result;
